...
GraphicsRenderer::GraphicsRenderer(PixelsBuffer &buffer)
    : _buffer(buffer)
{
}
...
void GraphicsRenderer::DrawLine(int x1, int y1, int x2, int y2,
                                const Color &color)
{
...
void GraphicsRenderer::DrawLine(const pri::PointPrimitive &start,
...
{
    DrawPoint(static_cast<int>(std::round(point.X())), 
              static_cast<int>(std::round(point.Y())), color);
}
...
{
    DrawLine(static_cast<int>(std::round(start.X())), 
             static_cast<int>(std::round(start.Y())),
             static_cast<int>(std::round(end.X())), 
             static_cast<int>(std::round(end.Y())), color);
}
...
{
    DrawAntialiasedLine(static_cast<int>(std::round(start.X())), 
                        static_cast<int>(std::round(start.Y())),
                        static_cast<int>(std::round(end.X())), 
                        static_cast<int>(std::round(end.Y())), color);
}
