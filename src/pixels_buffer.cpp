...
PixelsBuffer::PixelsBuffer(int width, int height) : _width(width), _height(height), _pitch(width * 4)
{
    assert(width > 0 && height > 0);
    _pixel_data.resize(static_cast<size_t>(width) * static_cast<size_t>(height));
    Clear(); // 默认清除为透明黑色
}
