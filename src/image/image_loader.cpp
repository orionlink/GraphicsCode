...
namespace image
{
    ImageLoader::ImageData ImageLoader::Load(const std::string& file_path, int desired_channels)
    {
        ...
        for (size_t i = 0; i < pixel_count; ++i)
        {
            ...
            if (actual_channels >= 1) r = data[byte_offset];
            if (actual_channels >= 2) g = data[byte_offset + 1];
            if (actual_channels >= 3) b = data[byte_offset + 2];
            if (actual_channels >= 4) a = data[byte_offset + 3];

            // 打包为 SDL_PIXELFORMAT_RGBA8888 格式（与 Color 类一致）
#if IMAGE_LITTLE_ENDIAN
            // 小端：R在最高字节，A在最低字节
            pixels[i] = (static_cast<uint32_t>(r) << 24) |
    ...
    {
        int width, height, channels;
        unsigned char* data = stbi_load_from_memory(buffer, static_cast<int>(length), &width, &height, &channels, desired_channels);
