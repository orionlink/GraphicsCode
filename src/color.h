...
class Color
{
  public:
    ...
    // 从 RGBA 分量构造
    Color(uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255) : _rgba(PackRGBA(r, g, b, a)) {}
    ...
    static Color Lerp(const Color& start, const Color& end, float t)
    {
        ...
        uint8_t r = t * static_cast<float>(end.R()) + (1.0f - t) * static_cast<float>(start.R());
        uint8_t g = t * static_cast<float>(end.G()) + (1.0f - t) * static_cast<float>(start.G());
        uint8_t b = t * static_cast<float>(end.B()) + (1.0f - t) * static_cast<float>(start.B());
        uint8_t a = t * static_cast<float>(end.A()) + (1.0f - t) * static_cast<float>(start.A());
