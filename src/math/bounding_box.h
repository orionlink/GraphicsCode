 ...
 */
template <class T> class BoundingBox2
{
  public:
    /**
     ...
     */
    BoundingBox2()
        : _min_x(std::numeric_limits<T>::max()), _min_y(std::numeric_limits<T>::max()),
          _max_x(std::numeric_limits<T>::lowest()), _max_y(std::numeric_limits<T>::lowest())
    {
    }
     ...
     * @param max_y 最大y坐标
     */
    BoundingBox2(T min_x, T min_y, T max_x, T max_y) : _min_x(min_x), _min_y(min_y), _max_x(max_x), _max_y(max_y) {}
     ...
     * @return 边界框
     */
    static BoundingBox2 FromPoints(const std::vector<Point2<T>>& points)
    {
        if (points.empty())
        {
            return BoundingBox2();
    ...
    {
        return point.X() >= _min_x && point.X() <= _max_x && point.Y() >= _min_y && point.Y() <= _max_y;
    }
    ...
    {
        return _max_x > other._min_x && _max_y > other._min_y && _min_x < other._max_x && _min_y < other._max_y;
    }
