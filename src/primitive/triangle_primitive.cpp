...
{
    // 计算三角形的面积（使用两条边的叉积）
    math::Vector2f edge1(static_cast<float>(_p0.X() - _p1.X()),
                         static_cast<float>(_p0.Y() - _p1.Y()));
    math::Vector2f edge2(static_cast<float>(_p0.X() - _p2.X()),
    ...
    // 计算点 P 到各顶点形成的三角形面积
    math::Vector2f pv0(static_cast<float>(x - _p0.X()),
                       static_cast<float>(y - _p0.Y()));
    math::Vector2f pv1(static_cast<float>(x - _p1.X()),
                       static_cast<float>(y - _p1.Y()));
    math::Vector2f pv2(static_cast<float>(x - _p2.X()),
...
Color TrianglePrimitive::InterpolateColor(const BarycentricCoord &barycentric) const
{
    ...
    uint8_t r = static_cast<uint8_t>(
        barycentric[0] * _p0.GetColor().R() + 
        barycentric[1] * _p1.GetColor().R() +
        barycentric[2] * _p2.GetColor().R()
    );
    uint8_t g = static_cast<uint8_t>(
        barycentric[0] * _p0.GetColor().G() + 
        barycentric[1] * _p1.GetColor().G() +
        barycentric[2] * _p2.GetColor().G()
    );
    uint8_t b = static_cast<uint8_t>(
        barycentric[0] * _p0.GetColor().B() + 
        barycentric[1] * _p1.GetColor().B() +
...
math::Point2f TrianglePrimitive::InterpolateUV(const BarycentricCoord &barycentric) const
{
    ...
    // 使用重心坐标插值 UV 坐标
    float u = barycentric[0] * _uv0.X() + 
              barycentric[1] * _uv1.X() +
              barycentric[2] * _uv2.X();
    
    float v = barycentric[0] * _uv0.Y() + 
              barycentric[1] * _uv1.Y() +
              barycentric[2] * _uv2.Y();
