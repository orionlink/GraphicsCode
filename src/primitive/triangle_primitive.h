...
class TrianglePrimitive : public IPrimitive
{
  public:
    ...
    TrianglePrimitive(int x1, int y1, int x2, int y2, int x3, int y3, const Color& color)
        : _p0(x1, y1, color), _p1(x2, y2, color), _p2(x3, y3, color)
    {
    }

    TrianglePrimitive(const math::Point2i& p1, const math::Point2i& p2, const math::Point2i& p3, const Color& color)
     ...
     */
    TrianglePrimitive(const PointPrimitive& p1, const PointPrimitive& p2, const PointPrimitive& p3,
                      std::shared_ptr<texture::Texture> texture, const math::Point2f& uv0, const math::Point2f& uv1,
                      const math::Point2f& uv2)
        : _p0(p1), _p1(p2), _p2(p3), _uv0(uv0), _uv1(uv1), _uv2(uv2)
    {
        _texture = texture;
    }
     ...
     */
    void SetTexture(std::shared_ptr<texture::Texture> texture, const math::Point2f& uv0, const math::Point2f& uv1,
                    const math::Point2f& uv2)
    {
